<template>
  <div>
    ...
    >
      ...
      <ul class="detalisList padd">
        <li class="clearfix row" :class="{'grey-bg': index % 2 == 0}" v-for="item,index in getRedEnvelopesList">
          <div>
            <strong>领取用户: {{item.sm_ui_GetName}}</strong>
  ...
  export default {
    ...
    methods: {
      ...
      initData(id, rows, state) {
        ...
        return this.$store.dispatch('getRedEnvelopesDetails', getShare)
      },
